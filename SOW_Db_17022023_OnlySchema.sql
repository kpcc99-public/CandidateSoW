...
END
GO
/****** Object:  StoredProcedure [dbo].[DashboardStats]    Script Date: 17-02-2023 14:24:09 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
/****** Object:  StoredProcedure [dbo].[DashboardCandidateStats]    Script Date: 20-02-2023 16:00:12 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT 'SO' as Category, C.STATUS AS [NAME],count(*) AS [Count]  FROM [dbo].[Candidate] C 
										   ...
										   WHERE (datetrunc(day,C.JoiningDate) >= @StartDate AND datetrunc(day,C.JoiningDate) <= @EndDate ) and C.isDeleted=0   
										   GROUP BY STATUS
UNION ALL
SELECT DISTINCT 'Location' as Category, LOCATION AS [NAME], count(*) AS [Count] FROM [dbo].[Candidate] where (datetrunc(day,JoiningDate) >= @StartDate AND  datetrunc(day,JoiningDate) <= @EndDate ) and isDeleted=0   
                                                                 GROUP BY LOCATION
End
GO
